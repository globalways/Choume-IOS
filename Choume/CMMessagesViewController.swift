...
class CMMessagesViewController: BaseViewController {
    ...
    func sendRequest(){
        CMContext.sharedInstance.isTokenLegal(){ (isTokenLegal) -> Void in
            if isTokenLegal {
                let loginData = CMContext.sharedInstance.getLoginData()
    ...
    private func getMessageContent(messageID: AnyObject, indexPath: NSIndexPath, completion: (() -> Void)?){
        if let loginData = CMContext.sharedInstance.getLoginData() {
            let userID = loginData["uid"].stringValue
            let token = loginData["token"].stringValue
...
extension CMMessagesViewController: DraggableViewDelegate {
    
    // MARK: - reply message
    private func readyToReplyMessage() {
        if let info = CMContext.sharedInstance.getLoginData() {

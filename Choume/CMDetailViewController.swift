...
class CMDetailViewController: BaseViewController, UIGestureRecognizerDelegate {
    ...
    override func cornerActionButtonDidTap() {
        ...
        CMContext.sharedInstance.isTokenLegal(){ (isTokenLegal) -> Void in
            if isTokenLegal{
                ...
                if let vc = mainStoryboard.instantiateViewControllerWithIdentifier("iBBSCommentViewController") as? CMCommentViewController{
                    vc.post_id = post_id
                ...
                CMContext.sharedInstance.login(cancelled: nil, completion: {
...
extension CMDetailViewController {
    // MARK: - refresh
    func refreshData(){
